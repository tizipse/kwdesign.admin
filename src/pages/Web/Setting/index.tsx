...
const System: React.FC = () => {
  ...
  const onSubmit = (values: object) => {
    ...
    for (const key in values) {
      ...
      if (Array.isArray(values[key])) {
        data[key] = values[key].length > 0 ? values[key][0]?.thumbUrl : '';
      } else {
        data[key] = values[key]
      }
    }
  ...
  const onChange = (value: any, values: object) => {
  ...
  const RenderPicture = (record: APIWebSetting.Data) => (
    <Form.Item name={record.key} key={record.id} label={record.label} valuePropName="fileList"
               getValueFromEvent={onUpload} rules={[{required: record.required === 1}]}
    >
      <Upload name="file" listType="picture-card" maxCount={1} action={Constants.Upload}
              headers={{Authorization: localStorage.getItem(Constants.Authorization) as string,}}
              data={{dir: '/pictures'}} onPreview={file => onPreview(file, record.label)}>
        {
  ...
  const RenderInput = (record: APIWebSetting.Data) => (
    <Form.Item name={record.key} key={record.id} label={record.label} rules={[{required: record.required === 1}]}>
      <Input/>
    </Form.Item>
  )

  const RenderEmail = (record: APIWebSetting.Data) => (
    <Form.Item name={record.key} key={record.id} label={record.label}
  ...
  return (
    <>
      <Card title="系统设置">
        <Spin spinning={load}>
          <Form
            form={former}
            labelCol={{span: 5}}
            disabled={!initialState?.permissions || initialState?.permissions?.indexOf('web.setting.save') < 0}
            onFinish={onSubmit}
      ...
      <Modal open={preview.visible} title={preview.title} centered footer={null}
