...
import {Button, Card, Image, notification, Popconfirm, Space, Switch, Table, Tag, Tooltip} from 'antd';
    ...
    doPaginate()
      .then((response: APIResponse.Paginate<APIWebBanners.Data[]>) => {
        if (response.code === Constants.Success) {
          ...
          setPaginate({total: response.data.total, size: response.data.size, page: response.data.page})
        }
      })
    ...
    doEnable(enable)
      .then((response: APIResponse.Response<any>) => {
        if (response.code !== Constants.Success) {
          ...
          if (data) {
            const temp = [...data];
            Loop.ById(temp, record.id, (item: APIWebBanners.Data) => (item.is_enable = enable.is_enable),);
            setData(temp);
          }
        }
      })
