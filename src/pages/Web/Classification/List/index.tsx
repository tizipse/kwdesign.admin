  ...
  const onDelete = (record: APIWebClassifications.Data) => {
    if (data) {
      ...
      Loop.ById(
        temp,
        record.id,
        (item: APIWebClassifications.Data) => (item.loading_deleted = true),
      );
      setData(temp);
    }
  ...
  const onEnable = (record: APIWebClassifications.Data) => {
    if (data) {
      ...
      Loop.ById(
        temp,
        record.id,
        (item: APIWebClassifications.Data) => (item.loading_enable = true),
      );
      setData(temp);
    }

    const enable: APIWebClassifications.Enable = {
      id: record.id,
      ...
      <Card
        ...
        extra={
          <Space size={[10, 10]} wrap>
            ...
            <Authorize permission="web.classification.create">
              <Tooltip title="创建">
        ...
        <Table dataSource={data} rowKey="id" loading={load} pagination={false}>
          ...
          <Table.Column
            ...
            render={(record: APIWebClassifications.Data) => (
              <Authorize
                permission="web.classification.enable"
          ...
          <Table.Column
            ...
            render={(record: APIWebClassifications.Data) => (
              <>
                <Authorize permission="web.classification.update">
                  <Button type="link" onClick={() => onUpdate(record)}>
                    编辑
                  </Button>
                </Authorize>
                <Authorize permission="web.classification.delete">
                  <Popconfirm
                    title="确定要删除该数据?"
                    placement="leftTop"
